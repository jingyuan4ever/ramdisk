...
*/
  ...
  retval = READ (fd1,fd, &address, sizeof(data1));
  ...
  retval = READ (fd1,fd, &address, sizeof(data2));
  ...
  retval = READ (fd1,fd, &address, sizeof(data3));
  ...
  while ((retval = READDIR (fd1,fd,&address))) { /* 0 indicates end-of-file */
    ...
    index_node_number = *((int*)&address[14]);
    printf ("Contents at addr: [%s,%d]\n", address, index_node_number);
  }
#endif // USE_RAMDISK
#endif // TEST4
